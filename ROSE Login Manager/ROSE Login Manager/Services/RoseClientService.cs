...
namespace ROSE_Login_Manager.Services
{
    ...
    public class RoseClientService : IRoseClientService
    {
        ...
        /// <summary>
        /// Gets installation path from a specific registry key
        /// </summary>
        private string? GetInstallPathFromRegistry(string keyPath)
        {
            try
            {
                ...
                if (key != null)
                {
                    // Try different value names that might contain the install path
                    var valueNames = new[] { "InstallLocation", "InstallPath", "Path", "Location" };
            ...
            {
                _logger.LogDebug(ex, "Error reading registry key: {KeyPath}", keyPath);
            }
        ...
        private string? GetSteamInstallPath()
        {
            ...
            {
                _logger.LogDebug(ex, "Error reading Steam registry key");
            }
        ...
        public void DebugRegistryEntries()
        {
            try
            {
                ...
                var otherKeys = new[]
                {
                    @"SOFTWARE\Rose Online",
                    @"SOFTWARE\WOW6432Node\Rose Online",
                    @"SOFTWARE\Gravity\Rose Online",
                    @"SOFTWARE\WOW6432Node\Gravity\Rose Online"
                };

                foreach (var keyPath in otherKeys)
                {
                    using var key = Registry.LocalMachine.OpenSubKey(keyPath);
                    if (key != null)
                    {
                        _logger.LogInformation("Found Rose-related registry key: {Key}", keyPath);
                        foreach (var valueName in key.GetValueNames())
                        {
                            var value = key.GetValue(valueName);
                            _logger.LogInformation("  {ValueName}: {Value}", valueName, value);
                        }
                    }
                }

                _logger.LogInformation("=== End Registry Debug ===");
            }
            catch (Exception ex)
            {
